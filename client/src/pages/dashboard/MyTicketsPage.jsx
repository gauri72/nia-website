...
const CURRENT_EVENT_SLUG = 'christmas-gala-2026';
...
export default function MyTicketsPage() {
  ...
  const currentBookings = galaLive ? bookings.filter((b) => b.eventSlug === CURRENT_EVENT_SLUG) : [];
  const pastBookings = galaLive ? bookings.filter((b) => b.eventSlug !== CURRENT_EVENT_SLUG) : [];
    ...
    <div>
      ...
      {galaLive ? (
        <div className="flex flex-col gap-6">
          <div>
            <h2 className="font-bold text-nia-navy-dark mb-3">Current Event</h2>
            {currentBookings.length === 0 ? (
              <p className="text-nia-text-faint">No current event tickets booked yet.</p>
            ) : (
              <div className="flex flex-col gap-3">{currentBookings.map(renderBookingCard)}</div>
            )}
          </div>
          {pastBookings.length > 0 && (
            <div>
              <h2 className="font-bold text-nia-navy-dark mb-3">Past Events</h2>
              <div className="flex flex-col gap-3">{pastBookings.map(renderBookingCard)}</div>
            </div>
          )}
        </div>
      ) : (
