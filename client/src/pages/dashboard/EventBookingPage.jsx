...
// Parameterized member-dashboard booking page — same ticket-only +
// join-and-book-bundle flow for any event in client/src/config/events.js.
// `event` is one entry from that registry ({slug, name, tickets}); `content`
// carries the event-card display copy (category label, description, date/
// time/venue strings) since that's marketing content, not registry data.
export default function EventBookingPage({ event, content }) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 flex flex-col gap-4">
          <Card>
            <span className="text-xs font-bold uppercase tracking-wide text-nia-orange">{content.category}</span>
            <h1 className="text-2xl font-extrabold text-nia-navy-dark mt-1">{content.title}</h1>
            <p className="text-nia-text-muted mt-2">{content.description}</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-5">
              <div className="flex items-start gap-2">
                <Calendar className="text-nia-orange flex-shrink-0 mt-0.5" size={18} />
                <div>
                  <p className="text-sm font-semibold text-nia-navy-dark">{content.date}</p>
                  <p className="text-xs text-nia-text-faint">{content.day}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <Clock className="text-nia-orange flex-shrink-0 mt-0.5" size={18} />
                <div>
                  <p className="text-sm font-semibold text-nia-navy-dark">{content.time}</p>
                  <p className="text-xs text-nia-text-faint">{content.timeSub}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <MapPin className="text-nia-orange flex-shrink-0 mt-0.5" size={18} />
                <div>
                  <p className="text-sm font-semibold text-nia-navy-dark">{content.venueName}</p>
                  <p className="text-xs text-nia-text-faint">{content.venueAddress}</p>
                  <a href={content.mapUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-nia-orange font-semibold hover:underline">View on map</a>
                </div>
              </div>
            </div>
          </Card>
