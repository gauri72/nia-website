  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        description={`${member.memberId} · Joined ${new Date(member.transactionDate || member.createdAt).toLocaleDateString()}${member.transactionDate ? ' (via Mollie transaction)' : ''}`}
        actions={<StatusBadge status={member.status === 'active' ? member.membershipStatus : member.status} />}
      />
          ...
          <Card>
            ...
            {isSuperAdmin ? (
              <div className="flex flex-col gap-2">
                <Button variant="secondary" disabled={member.status === 'active'} onClick={() => handleStatusChange('active')}>Activate</Button>
                <Button variant="secondary" disabled={member.status === 'suspended'} onClick={() => handleStatusChange('suspended')}>Suspend</Button>
                <Button variant="danger" disabled={member.status === 'deleted'} onClick={() => handleStatusChange('deleted')}>Delete Account</Button>
              </div>
            ) : (
